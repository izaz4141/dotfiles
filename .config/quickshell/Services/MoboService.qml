...
Singleton {
    ...
    function formatKib(kib: real): string {
        ...
        while (kib >= 1024 && unitIdx < units.length - 1) {
            kib /= 1024.0;
            unitIdx += 1;
        }

        return (unitIdx > 0)
            ? `${kib.toFixed(2)} ${units[unitIdx]}`
            : `${kib} ${units[unitIdx]}`;
    }
    ...
    Process {
        ...
        command: ["sh", "-c", "df | grep '^/dev/' | awk '{print $1, $3, $4, $6}'"]
        stdout: StdioCollector {
            onStreamFinished: {
                ...
                root.storageMap = Array.from(deviceMap, ([device, stats]) => ({
                    device: device,
                    mountPoint: stats.mountPoint,
                    used: stats.used,
                    total: stats.avail + stats.used,
                    percent: stats.used / (stats.avail + stats.used)
                }))
