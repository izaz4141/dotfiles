...
if [ -f $wallpapereffect ]; then
    ...
    if [ ! "$effect" == "off" ]; then
        ...
        if [ -f $generatedversions/$effect-$wallpaperfilename ] && [ "$force_generate" == "0" ] && [ "$use_cache" == "1" ]; then
            ...
            notify-send --replace-id=5 "Using wallpaper effect $effect..." "with image $wallpaperfilename" -h int:value:33
            source $HOME/.config/hypr/effects/wallpaper/$effect
            notify-send --replace-id=5 "Finish generating wallpaper effect $effect..." "with image $wallpaperfilename" -h int:value:100
        fi
...
if type nwg-dock-hyprland >/dev/null 2>&1; then
...
# Update Cava
# -----------------------------------------------------
source ~/.config/cava/colors.sh
sed -i "s/^gradient_color_1 =.*/gradient_color_1 = '$color2'/" ~/.config/cava/config
sed -i "s/^gradient_color_2 =.*/gradient_color_2 = '$color3'/" ~/.config/cava/config
sed -i "s/^gradient_color_3 =.*/gradient_color_3 = '$color4'/" ~/.config/cava/config
sed -i "s/^gradient_color_4 =.*/gradient_color_4 = '$color5'/" ~/.config/cava/config
sed -i "s/^gradient_color_5 =.*/gradient_color_5 = '$color6'/" ~/.config/cava/config
killall -USR2 cava
